...
void calcMediaResiduos() {
	...
	media = soma / totalRegistosIntroduzidos;
	printf("A média de resíduos é: %.2f", media);
}
...
void listartEcopontosTipo(int tipo) {
	...
	// For para limpar a variavel "tipoEcoponto"
	for(i = 0; i != '\0'; i++) {
		free(&tipoEcoponto[i]);
	}
	tipoEcoponto = NULL;
...
void calcRota() {
						...
						if (totalCoordenadas == 1) {
							...
							if (fp == NULL) {
								...
								if (resposta == 's' || resposta == 'S') {
									...
									printf("Coordenadas iniciais, x: %.2f; y: %.2f", coordenadasAPerrcorrer[0].x, coordenadasAPerrcorrer[0].y);
								} else {
									printf("A cancelar ...");
								}
							} else {
								...
								fprintf(fp, "Coordenadas iniciais, x: %.2f; y: %.2f", coordenadasAPerrcorrer[0].x, coordenadasAPerrcorrer[0].y);
	...
	for (i = 0; i < totalRegistosIntroduzidos + 1; i++) {
		free(distancia[i]); distancia[i] = NULL;
	}
	free(distancia); 
    distancia = NULL;
}
